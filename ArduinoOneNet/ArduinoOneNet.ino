...
#define PASSWORD "6e7e68c2c8c1"//填写自己的WIFI密码
#define HOST_NAME "api.heclouds.com"  //API主机名称，连接到OneNET平台，无需修改
#define DEVICE_ID "644630400"       //填写自己的OneNet设备ID
#define HOST_PORT (80)                //API端口，连接到OneNET平台，无需修改
String APIKey = "Ye6s4bqZnoF93bGA"; //与设备绑定的APIKey
 ...
 * 但接ESP8266时必须恰好相反
 *********************/
SoftwareSerial ySerial(3,2);
ESP8266 wifi(ySerial);
...
void setup()
{
  ...
  ySerial.println("AT+UART_CUR=9600,8,1,0,0");
  ySerial.begin(9600);
...
void loop(){
  ...
  if(i>=5) i=0;else i++;
    ...
    float sensor_pm25 = (float)pm25;
    float sensor_pm10 = (float)pm25;
    float sensor_hum = (float)DHT11.humidity;
    float sensor_tem = (float)DHT11.temperature;
    Serial.print("pm2.5: ");
    Serial.println(sensor_pm25, 2);

    Serial.print("pm10: ");
    Serial.println(sensor_pm10, 2);
    ...
    if (wifi.createTCP(HOST_NAME, HOST_PORT)) { //建立TCP连接，如果失败，不能发送该数据
      ...
      jsonToSend += "\"" + String(buf) + "\"";
      jsonToSend += ",\"PM2.5\":";
      dtostrf(sensor_pm25, 1, 2, buf);
      jsonToSend += "\"" + String(buf) + "\"";
      jsonToSend += ",\"PM10\":";
      dtostrf(sensor_pm10, 1, 2, buf);
      jsonToSend += "\"" + String(buf) + "\"";
      jsonToSend += "}";
